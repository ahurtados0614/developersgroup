(function($) {
    ...
    // 💖 Fecha y hora exacta de la boda (AÑO, MES-1, DÍA, HORA, MINUTO, SEGUNDO)
    // Ejemplo: 20 de octubre de 2025 a las 18:00 (6:00 p.m.) para el mes usa uno menos
    const fechaBoda = new Date(2025, 11, 19, 18, 0, 0);
    const ahora = new Date();

    // 🕒 Si la boda todavía no ha ocurrido → cuenta regresiva
    if (ahora < fechaBoda) {
        ...
        // 💞 Si la boda ya pasó → muestra tiempo transcurrido

        function actualizarTiempo() {
            const ahora = new Date();
            let diff = Math.floor((ahora - fechaBoda) / 1000); // diferencia en segundos
            ...
            $("#clock").html(
                '<div class="box"><div>' + days + '</div><span>Días</span></div>' +
                '<div class="box"><div>' + hours.toString().padStart(2, '0') + '</div><span>Horas</span></div>' +
                '<div class="box"><div>' + minutes.toString().padStart(2, '0') + '</div><span>Minutos</span></div>' +
                '<div class="box"><div>' + seconds.toString().padStart(2, '0') + '</div><span>Segundos</span></div>'
            );
        }
